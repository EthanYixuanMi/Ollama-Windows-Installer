...
class OllamaInstallerApp:
    ...
    def create_download_frame(self):
        ...
        # ➕ 新增：本地版本安装按钮
        local_install_btn = tk.Button(
            frame,
            text="📦 " + ("安装本地版本" if self.language == "zh" else "Install Local Version"),
            font=("Arial", 12),
            height=2,
            width=40,
            command=self.install_local_version
        )
        local_install_btn.pack(pady=10)
    ...
    def install_local_version(self):
        try:
            ...
            if not os.path.exists(exe_path):
                messagebox.showerror(self.texts["title"], "本地安装包 OllamaSetup.exe 未找到！")
                return
            ...
            messagebox.showerror(self.texts["title"], f"无法启动本地安装程序：{e}")
